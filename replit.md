...
- **LP Login Page & Component**: New /lp/login page with dedicated LPSignIn component for investor-specific login flow, magic link authentication, redirect to /lp/dashboard
- **LP Route Protection**: All /lp/* routes now redirect unauthenticated users to /lp/login (not main login), with proper next parameter handling
...
- **Explicit LP Role Assignment**: /api/lp/register now explicitly sets role: "LP" on user creation

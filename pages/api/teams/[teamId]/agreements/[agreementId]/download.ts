...
export default async function handle(
  ...
  if (req.method === "POST") {
    ...
    try {
      ...
      // Check if the content is a BF Fund URL
      const isBFFundUrl =
        agreement.content.includes("bermudafranchisegroup.com/view/") ||
        agreement.content.includes("bffund.com/view/");
      ...
      if (isBFFundUrl) {
        // Extract linkId from BF Fund URL
        const urlParts = agreement.content.split("/view/");
        if (urlParts.length < 2) {
          return res.status(400).json("Invalid BF Fund URL format");
        }

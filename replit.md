...
*   **Role Hierarchy:** Teams have three admin-level roles: SUPER_ADMIN (one per team, can remove admins), ADMIN (full access except removing admins), and MEMBER. Only the super admin can remove other admins. Super admins cannot be removed - they must transfer the role first. The initial super admin for Bermuda Franchise Fund is investors@bermudafranchisegroup.com.

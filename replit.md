...
*   **Admin and GP Dashboards**: Features include fund settings with NDA gate toggling, comprehensive fund overviews with financial aggregates (Total Raised, Distributed, Commitments), Recharts visualizations, and a bulk action wizard for capital calls or distributions. **Dual Threshold System**: Initial Closing Threshold (gates capital calls until met) vs Full Authorized Amount (progress tracking only) with color-coded UI cards and tooltips.
